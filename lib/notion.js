...
// ── Lycan Lore ──────────────────────────────────────────────
const LYCAN_LORE_DB_ID = process.env.NOTION_LYCAN_LORE_DB_ID;

export async function getLycanLoreEntries() {
  if (!LYCAN_LORE_DB_ID) return [];
  const response = await fetch(
    `https://api.notion.com/v1/databases/${LYCAN_LORE_DB_ID}/query`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${NOTION_TOKEN}`,
  ...
  return data.results.map((page) => {
    ...
    return {
      ...
      era: getText(props.Era),
      eraId: getText(props.Era)?.toLowerCase().replace(/\s+/g, '-'),
      year: getText(props.Year),
      tag: getText(props.Tag),
      tagStyle: getText(props.TagStyle) || 'origin',
      tag2: getText(props.Tag2),
      tag2Style: getText(props.Tag2Style) || 'war',
      body: getText(props.Body),
      quote: getText(props.Quote),
      quoteAuthor: getText(props.QuoteAuthor),
      major: props.Major?.checkbox || false,
      coverImage,
    };
  });
}

export async function getLycanEras() {
  const entries = await getLycanLoreEntries();
  const seen = new Set();
  const eras = [];
  for (const entry of entries) {
    if (!seen.has(entry.eraId)) {
