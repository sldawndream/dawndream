...
export default function EventsPage({ events }) {
  ...
  const upcomingEvents = events.filter(e => {
    ...
    return new Date(e.date + 'T00:00:00') >= new Date(today.toDateString());
  });
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="DawnDream events — gatherings, wars and story nights in Second Life." />
      </Head>
      ...
      <div className={styles.body}>
        ...
        <div className={styles.calWrap}>
          ...
          <div className={styles.calGrid}>
            ...
            {Array.from({ length: firstDay }).map((_, i) => (
              <div key={`empty-${i}`} className={`${styles.calDay} ${styles.empty}`} />
            ))}
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div key={day} className={`${styles.calDay} ${isToday ? styles.today : ''}`}>
                  ...
                  {dayEvents.map(e => (
                    <div key={e.id} className={`${styles.calEvent} ${e.eventType === 'Social' ? styles.calEventSocial : e.eventType === 'PVP' || e.eventType === 'War' ? styles.calEventPvp : ''}`}>
                      {e.name}
                    </div>
                  ))}
                </div>
            ...
            <div className={styles.eventsGrid}>
              {upcomingEvents.map(event => (
                <div key={event.id} className={styles.eventCard}>
                  {event.bannerImage ? (
                    <img src={event.bannerImage} alt={event.name} className={styles.eventBanner} />
                  ) : (
                    <div className={styles.eventBannerPlaceholder}>Event Banner</div>
                  )}
            ...
            <div className={styles.eventsGrid}>
              {pastEvents.map(event => (
                <div key={event.id} className={`${styles.eventCard} ${styles.pastCard}`}>
                  {event.bannerImage ? (
                    <img src={event.bannerImage} alt={event.name} className={`${styles.eventBanner} ${styles.pastBanner}`} />
                  ) : (
                    <div className={styles.eventBannerPlaceholder}>Event Banner</div>
                  )}
                  <div className={styles.eventBody}>
        ...
        {events.length === 0 && (
          <p style={{ color: '#7a5a50', fontStyle: 'italic' }}>No events yet — add them in your Notion database!</p>
        )}
      </div>
