...
export async function getPlayerByToken(token) {
  ...
  // Only check expiry if expires_at is actually set
...
export async function getAllPlayers() {
  ...
  const { data } = await supabase
    .from('players')
    .select('id, avatar_name, status, role, created_at, approved_at')
    .order('created_at', { ascending: false });
  return data || [];
}

// Helper — true for both admin and owner
export function isAdmin(role) {
  return role === 'admin' || role === 'owner';
}

// Helper — true only for owner
