...
export async function getServerSideProps({ req }) {
  try {
    ...
    const sanitised = hordes.map(horde => ({
      ...horde,
      warLord:   isLoggedIn ? horde.warLord   : null,
      firebrand: isLoggedIn ? horde.firebrand : null,
      ravager:   isLoggedIn ? horde.ravager   : null,
    }));

    return { props: { hordes: sanitised, isLoggedIn } };
  } catch (err) {
    return { props: { hordes: [], isLoggedIn: false } };
  }
}

export default function HordesPage({ hordes, isLoggedIn }) {
  return (
    <>
      <Head>
      ...
      <div className={styles.warningBar}>⚠ These factions operate outside the law of the Eternal Throne — they are not recognised, not protected, and not forgiven.</div>
      ...
      <div className={styles.body}>
        ...
        {!isLoggedIn && (
          <p className={styles.rankNotice}>
            🔒 <a href="/login" style={{ color: '#c0392b', textDecoration: 'none' }}>Login</a> or <a href="/register" style={{ color: '#c0392b', textDecoration: 'none' }}>register</a> to see horde leadership.
          </p>
        )}

        {hordes.length === 0 && (
          <p style={{ color: '#7a5a50', fontStyle: 'italic' }}>No hordes yet.</p>
        )}
              ...
              <div className={styles.hordeHeader}>
                ...
                <div className={styles.hordeInfo}>
                  ...
                  <div className={styles.hordeRanks}>
                    {isLoggedIn ? (
                      <>
                        <span className={`${styles.rankPill} ${styles.rankWl} ${!horde.warLord ? styles.rankEmpty : ''}`}>⚔ WarLord: {horde.warLord || 'None'}</span>
                        <span className={`${styles.rankPill} ${styles.rankFb} ${!horde.firebrand ? styles.rankEmpty : ''}`}>Firebrand: {horde.firebrand || 'None'}</span>
                        <span className={`${styles.rankPill} ${styles.rankRv} ${!horde.ravager ? styles.rankEmpty : ''}`}>Ravager: {horde.ravager || 'None'}</span>
                      </>
                    ) : (
                      <span className={styles.rankLocked}>🔒 Login to view leadership</span>
                    )}
                  </div>
                  ...
                  {horde.lore && (
                    <details className={styles.loreDetails}>
                      ...
                      <p className={styles.hordeLore}>{horde.lore}</p>
                    </details>
                  )}
                </div>
              </div>
